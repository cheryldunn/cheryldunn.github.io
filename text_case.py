...
def normalize(tokens):
    #takes a list of tokens and returns a list of tokens that has been normalized by lowercasing all tokens and removing Project Gutenberg frontmatter

#lowercase all words
    normalized = [token.lower() for token in tokens]

#very rough end of front frontmatter
    end_of_front_matter = 90
#very rough beginning of end end_of_front_matter
    start_of_end_matter = -2973
#get only the text between the end matter and front end_of_front_matter
...
bronte = [normalize(text) for texxt in bronte]
...
def print_top_words(freq_dist_text):
    #Takes a frequency distribution of a text and prints out the top ten words in it.
    print('=====')
